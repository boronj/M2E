...
#Setup up CUDA 12.4
if [ ! -e "$DIRECTORY/cuda_12.4.0_550.54.14_linux.run" ]; then
...
echo "A" | sudo wget https://github.com/facebookresearch/meshtalk/releases/download/pretrained_models_v1.0/pretrained_models.zip -O "$DIRECTORY"/PTM.zip --no-check-certificate --continue
unzip "$DIRECTORY/PTM.zip" -d "$DIRECTORY/meshtalk_models"
